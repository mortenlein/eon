...
import { getPlayerSubtitleOverrides } from '/hud/gsi/helpers/player-subtitle-overrides.js'
...
const hasUsableObserverData = (player) => {
	return !!(
		player?.state
		|| player?.match_stats
		|| (player?.weapons && Object.keys(player.weapons).length)
	)
}
...
export const parsePlayers = () => {
	...
	const playerSubtitleOverrides = getPlayerSubtitleOverrides()
	const { hiddenPlayerNames, hiddenPlayerSteam64Ids } = getHiddenPlayers()
	const hideCoachesByName = options['teams.hideCoachesByName']
	const playerEntries = Object.entries(gsiState.allplayers || {})

	if (! playerEntries.length && gsiState.player?.steamid && hasUsableObserverData(gsiState.player)) {
		playerEntries.push([gsiState.player.steamid, gsiState.player])
	}
	...
	for (const [steam64Id, player] of playerEntries) {
		...
		if (observerSlot === undefined && gsiState.allplayers) continue
		...
		const mvps = player.match_stats?.mvps ?? 0
		const score = player.match_stats?.score ?? 0
		const equipmentValue = player.state?.equip_value ?? 0
		const roundDamage = player.state?.round_totaldmg ?? 0
		const roundHeadshotKills = player.state?.round_killhs ?? 0
